    ...
    bs4TabItems(
      bs4TabItem(
        ...
        mainPanel(absolutePanel(width = 20,bottom = 1, right = -150, top = -100,
                                img(src = "https://linkpicture.com/q/GTD.png" , height = 1000, width = 400)
        ),
        ...
        br(),
        h2(span("Characteristics of the GTD ", style = 'color:red')),
        tags$ul(
      ...
      bs4TabItem(
        ...
        fluidRow(
          bs4Card(
            ...
            collapsible = TRUE,
            dataTableOutput(outputId = "showData"))
      ...
      bs4TabItem(
        ...
        bs4TabSetPanel(id = "tab", side = "center",
                       bs4TabPanel(tabName = "WORLD PANEL-1",
                                   ...
                                   fluidRow(
                                     bs4Card(
                                       title = "WORLD MAP OF ATTACKS TYPE",
                                       closable = FALSE,
                                       width = 12,
                                       solidHeader = TRUE,
                                       status = "success",
                                   ...
                                   fluidRow(
                                     ...
                                     bs4Card(
                                       title = "TOTAL-ATTACKS - REGION_WISE",
                                       closable = FALSE,
                                       width = 6,
                                       solidHeader = TRUE,
                                       status = "danger",
        ...
        bs4TabSetPanel(id = "tabs", side = "center",
                       bs4TabPanel(tabName = "INDIA PANEL-1",
                                   ...
                                   fluidRow(
                                     ...
                                     bs4Card(
                                       title = "TOTAL-ATTACKS - STATE_WISE",
                                       closable = FALSE,
                                       width = 6,
                                       solidHeader = TRUE,
                                       status = "danger",
      ...
      bs4TabItem(
        ...
        bs4Card(
          ...
          tags$ul(
            ...
            img(src = "https://i.ibb.co/0M8f6F4/image1.png" , height = 100, width = 400),
            br(),
            br(),
            ...
            br(),
            h3("Here, the Level of Significance (Î±) is important. "),
            br(),
            h3("If p-value <= alpha : Significant result â reject null hypothesis (H0)"),
            p(""),
            h3("If p-value > alpha : Not a significant result â fail to reject null hypothesis (H0)"),
           br(),
           br(),
           ...
           br(),
            ...
            h3(strong("The P-value (2.2e-16) is less than the significance level (0.05). Therefore, we reject the null hypothesis. ")),
            p(""),
           h3(strong("Contingency table can be visualized using the function balloonplot() in gplots package. This function draws a graphical matrix where each cell contains a dot whose size reflects the relative magnitude of the corresponding component. ",align ="justify")),
        )),
        bs4Card(
          title = "BALLOON - PLOT",
          closable = FALSE,
...
server <- function(input, output, session) {
  ...
  cull <- reactive({
    firstData <- 
      filter(df[order(Type %in% input$Type, decreasing=TRUE)[Value >= input$years[1] & Value <= input$years[2]],])
  ...
  output$map <- renderLeaflet({
    leaflet(Dataset) %>% addTiles() %>%
      addMarkers(data = filteredData(),popup = ~paste(as.character(iyear),",","Total Attacks",as.character(attacktype1_txt)), clusterOptions = markerClusterOptions())
    
    leaflet(Dataset)%>% addTiles() %>% 
      addMarkers(data = filteredData(),popup = ~paste(as.character(iyear),",","Total Attacks",as.character(attacktype1_txt)), clusterOptions = markerClusterOptions(freezeAtZoom = 6))
    leaflet(Dataset) %>%  addTiles() %>% 
  ...
  output$indiamap <- renderLeaflet({
    leaflet(Dataset2) %>% addTiles() %>%
      addMarkers(data = filteredData2(),popup = ~paste(as.character(iyear),",","Total Attacks",as.character(attacktype1_txt)), clusterOptions = markerClusterOptions())
    
    leaflet(Dataset2)%>% addTiles() %>% 
      addMarkers(data = filteredData2(),popup = ~paste(as.character(iyear),",","Total Attacks",as.character(attacktype1_txt)), clusterOptions = markerClusterOptions(freezeAtZoom = 6))
    leaflet(Dataset2) %>%  addTiles() %>% 
